...
import {
  countCommas,
  splitAddressMeshed,
  splitOneCommaAddress,
  splitTwoCommaAddress,
  fullAddress,
  splitThreeCommaAddress,
} from '../util/splitAddress';
import { findStateName } from '../util/findStateName';
import { stateHouseWsInterface } from './wsModels/stateHouse.model';
import { candNewInterface } from './wsModels/candNew.model';
...
type wsModels =
  ...
  | AyaResWsModel
  | CalStateWsInterface
  | CrisTravisWsInterface
  | DelgadoWsInterface
  | CristinaWsInterface
  | EmilyStateWsInterface
  | JacobResWsInterface
  | RiResWsInterface
  | StateLegWsInterface
  | stateLegCOGAWsInterface
  | TravisWsInterface
  | stateHouseWsInterface;
...
export class BaseFileReader {
  ...
  goThroughWorkSheets() {
    this.wsNames.forEach((wsName: string) => {
      ...
      if (
        this.fn == ProjectNames.EmilyState ||
        this.fn == ProjectNames.StateLeg ||
        this.fn == ProjectNames.RiRes
      ) {
        this.range = 1;
      }
